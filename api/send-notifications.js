...
export default async function handler(req, res) {
  ...
  try {
    ...
    const hh = String(londonTime.getHours()).padStart(2, '0');
    const mm = String(londonTime.getMinutes()).padStart(2, '0');
    const currentTime = `${hh}:${mm}`;
    const todayStr = londonTime.toDateString();
    ...
    // Get all user data
    const users = await supabaseGet('user_data?select=email,tasks,last_date');
    ...
    for (const sub of subs) {
      ...
      try {
        ...
        if (lastDate) {
          ...
          const diffDays = Math.floor((londonTime - last) / (1000 * 60 * 60 * 24));
    ...
    res.status(200).json({ sent, time: currentTime, subs: subs.length });
  } catch(e) {
